    ...
    public function index()
{
    ...
    {
    $user_id = Auth::id();
    $user_book = UserBook::where('book_id', $id)->where('user_id', $user_id)->first();
    if ($user_book) {
        $user_book->delete();
        return redirect()->route('home')->with('success', '本の登録を削除しました。');
    } else {
        return redirect()->route('home')->with('error', '本の登録が見つかりませんでした。');
    }
}
}
